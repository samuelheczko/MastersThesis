...
for n in np.vstack([np.array([25,50,75,100,125,150,175,200]),np.array([75,150,225,300,375,450,525,600])]).T:
    ...
    #2 make df of the statistical values
            ...
            result_df.to_csv(path + f'results/ridge_regression/{CT}/gaby_results/ridge_results_yFS_n_feat_{n_feat}_bias_reduct_{bias_reduct}_prop_{prop}_{CT}_{current_atlas}_fold_size_{n_train}_paperDict.csv')
            preds_real_df.to_csv(path + f'results/ridge_regression/{CT}/gaby_results/ridge_preds_yFS_n_feat_{n_feat}_bias_reduct_{bias_reduct}_prop_{prop}_{CT}_{current_atlas}_fold_size_{n_train}_paperDict.csv')