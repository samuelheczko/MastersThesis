...
csv_paths  = glob.glob(path + f'/results/connectomes/{CT}_gabys/dictLearn/*.csv')
print(csv_paths)
...
for n_feat in np.array([1000]):   
    ...
    for data_path_i, data_path in enumerate(csv_paths): ##loop over atlases
        ...
        cognition = cognition, n_iter_search=n_iter,Feature_selection = Feature_selection,manual_folds = False,prop = prop,fold_list = folds_gaby2,n_test = n_test,n_train = n_train,z_score = False,bias_reduct = bias_reduct,n_loops = n_loops,fit_intercept = fit_intercept)
